...
export async function GET() {
  try {
    const allTimeAdvancedPlayers = await prisma.playerStats.findMany({
    ...
    const weaponStats = allTimeAdvancedPlayers.map((player) => {
    ...
    const transformedData = allTimeAdvancedPlayers.map((player, index) => {
      return {
        id: player.id,
        name: player.name,
        ...
        heGrenadesThrown: player.heGrenadesThrown,
        molotovsThrown: player.molotovsThrown,
        decoysThrown: player.decoysThrown,
        ...
        killsTroughWallPerGame: player.killsThroughWallPerGame,
        killsOnFlashPerGame: player.killsOnFlashPerGame,
        killsTroughSmokePerGame: player.killsThroughSmokePerGame,
        bombPlantsPerGame: player.bombPlantsPerGame,
        bombDefusesPerGame: player.bombDefusesPerGame,
        flashesThrownPerGame: player.flashesThrownPerGame,
        smokesThrownPerGame: player.smokesThrownPerGame,
        heGrenadesThrownPerGame: player.heGrenadesThrownPerGame,
        molotovsThrownPerGame: player.molotovsThrownPerGame,
        decoysThrownPerGame: player.decoysThrownPerGame,
        enemiesFlashedPerGame: player.enemiesFlashedPerGame,
        grenadeDamagePerGame: player.grenadeDamagePerGame,

        clutches1v1Played: player.clutches1v1Played,
        clutches1v1Won: player.clutches1v1Won,
        clutches1v2Played: player.clutches1v2Played,
        clutches1v2Won: player.clutches1v2Won,
    ...
    console.error("Error fetching all-time advanced players:", error);
    return NextResponse.json(
      { error: "Failed to fetch all-time advanced players" },
      { status: 500 }
    );
  }
}
