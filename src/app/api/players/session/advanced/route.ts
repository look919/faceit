...
export async function GET() {
  try {
    ...
    const weaponStats = sessionAdvancedPlayers.map((player) => {
    ...
    const transformedData = sessionAdvancedPlayers.map((player, index) => {
      return {
        id: player.id,
        name: player.name,
    ...
    return NextResponse.json(
      { error: "Failed to fetch session advanced players" },
      { status: 500 }
    );
  }
}
